...
def display_score():
	...
	score_surf = test_font.render(f'Score: {current_time}',False,(252, 246, 245))
...
def obstacle_movement(obstacle_list):
	if obstacle_list:
		for obstacle_rect in obstacle_list:
			...
			if obstacle_rect.bottom == 300: screen.blit(worm_surf,obstacle_rect)
...
while True:
	for event in pygame.event.get():
		...
		if game_active:
			if event.type == obstacle_timer:
				obstacle_group.add(Obstacle(choice(['bee','worm','worm','worm'])))

			if event.type == worm_animation_timer:
				if worm_frame_index == 0: 
					worm_frame_index = 1
				else: 
					worm_frame_index = 0
			worm_surf = worm_frames[worm_frame_index] 
	...
	if game_active:
		...
		screen.fill((138, 170, 229))
		screen.blit(player_stand,player_stand_rect)
		obstacle_rect_list.clear()
		player_rect.midbottom = (80,300)
		player_gravity = 0

		score_message = test_font.render(f'Your score: {score}',False,(252, 246, 245))
